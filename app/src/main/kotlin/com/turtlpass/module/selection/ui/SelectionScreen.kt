...
fun SelectionScreen(
    ...
    val sheetState = rememberModalBottomSheetState(
        ...
        confirmValueChange = { newValue ->
            // Only close if the user drags FAR enough
            if (newValue == ModalBottomSheetValue.Hidden) {
                false  // block accidental dismiss
            } else true
        },
    )
    ...
    BackHandler(sheetState.isVisible) {
        when (currentRoute) {
            NavigationDeviceItem.Selection.route -> finish()
            NavigationDeviceItem.UserAccounts.route -> popBackStack()
            NavigationDeviceItem.PIN.route -> popBackStack()
            NavigationDeviceItem.ConnectUsb.route -> popBackStack()
            NavigationDeviceItem.Loader.route -> popBackStack()
            null -> finish()
        }
    }
...
private fun Preview() {
    AppTheme {
        SelectionScreen(
            ...
            navBackStackEntry = rememberNavController().currentBackStackEntryAsState().value,
//            selectionUiState = remember { mutableStateOf(SelectionUiState(model = SelectionInput())) },
...
//            },
            userAccountUiState = remember { mutableStateOf(UserAccountUiState(accountsPermission = AccountsPermission.Rationale)) },
